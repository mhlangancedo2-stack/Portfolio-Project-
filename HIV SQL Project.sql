...
-- Q1. Name Analysis: List all unique Country names from the plwh table and the number of characters in each name. 
-- Order the results so the longest country names appear at the top.

select distinct country, length(country) as charactor_length
from plwh
order by charactor_length desc;

-- Q2. Regional Statistics: Using the adult_prevalence table, calculate the average Count_median for each WHO Region for the year 2018. 
-- Only include regions where the average is greater than 1.0.
...
-- Q3. High Mortality Search: Select the Country and Count_median from the hiv_deaths table where the count is greater than 5,000. 
...
-- Q4. Data Labeling & Union: Create a single list of all countries mentioned in both the art_coverage and pediatric_art tables. 
-- Add a column named Program_Type that labels records from the first table as "General ART" and the second as "Pediatric ART". 
-- Ensure there are no duplicate country-label pairs.
...
-- Q5. Performance Check: Identify regions in the pmtct_data table that have an average Percentage Recieved_median (PMTCT coverage) of 90% or higher.
...
-- Q6. Mortality Ratio Analysis (CTE): Construct a query using a Common Table Expression (CTE) that combines data from plwh and hiv_deaths for the year 2018. 
-- Calculate a "Mortality Ratio" for each country (Total Deaths / Total People Living with HIV). Return the Country, WHO Region, and the Ratio.
...
-- Q7. Regional Rankin: Using the art_coverage table, use a Window Function to rank countries within each WHO Region based on their Reported number of people receiving ART. 
...
-- Q8. Data Standardization & Join: Notice that the plwh table uses a column named WHO_Region (with an underscore), while art_coverage uses WHO Region (with a space). 
...
-- Q9. Identifying Data Gaps: Write a query to find countries in the art_coverage table that have 'Nodata' in the Estimated ART coverage (%) column.
...
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
-- Calculate the ratio of HIV-related deaths to the total number of people living with HIV (PLWH) for every country. 
-- Please rank these countries globally based on this ratio, ensuring we can see which countries are struggling the most despite their population size."
...
-- Can you provide a report that compares the average ART coverage percentage for children versus the general population across each WHO Region? 
-- I’m specifically looking for regions where the gap between these two metrics is greater than 15%
